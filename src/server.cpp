...
{
	std::cout << "This is from server" << std::endl;
	...
	dc->onMessage([&](auto data) {
			...
			if (std::holds_alternative<std::string>(data)) {
				std::cout << "[Received: " << std::get<std::string>(data) <<
					"], sending it back to client" << std::endl;
				dc->send(std::get<std::string>(data));
			}
		});
	...
	while (!exit)
	{
		...
		switch (command)
		{
			...
			case 3:
			{
				...
				std::cout << "Server does not input messages now, just replies what client sent" << std::endl;
				break;
			}
			case 4:
			{
